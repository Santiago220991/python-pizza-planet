...
from ..repositories.models import Ingredient, Order, OrderDetail, Size, Beverage, OrderBeverageDetail, db
...
class ReportController():
    ...
    def get_most_requested_ingredient():
        request_ingredients=OrderDetail.query.with_entities(OrderDetail.ingredient_id, func.count(OrderDetail.ingredient_id)).group_by(OrderDetail.ingredient_id).order_by(func.count(OrderDetail.ingredient_id).desc()).all()
        most_requested_ingredient=request_ingredients[0]
        ingredient_name=IngredientController.get_by_id(most_requested_ingredient[0])[0]["name"]
        ingredient_requested_times=most_requested_ingredient[1]
        return {"ingredient": ingredient_name, "requested_times": ingredient_requested_times}
    ...
    def create(cls):
        try:
            return {"most_requested_ingredient":cls.get_most_requested_ingredient(), 
                    "month_with_the_higest_revenue":cls.get_month_with_most_revenue(),
                    "best_clients":cls.get_best_costumers()}, None
